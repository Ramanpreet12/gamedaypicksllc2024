(function ($) {
    ...
    $.fn.dateTimePicker = function (options) {
        ...
        return this.each(function () {
            ...
            const last_date = $('.lastDate_Data').attr('data');
            if(last_date){
                var arrF = last_date.split(' ');
                var df = moment(arrF[0]).format("DD/MM/Y");
		

                var dt = arrF[1]+' '+arrF[2];
                var $s = $('<span>');
                //show current date on placeholder
                $s.text(df);
                elem.empty();
                elem.append($s);
                $s = $('<i>');
            ...
            function updateMainElemGlobal() {
                ...
                if (settings.showTime) {
                    $s = $('<span>');
                      //show current time on placeholder
            ...
            elem.on('click', function () {
                ...
                function createTimer() {
                    ...
                    $d.addClass('dtp_modal-digit');
                    $d.attr('id', 'd-mm');
                    $d.text(lastSelected.format('mm A'));
                    $panel.append($d);
                    $div.append($panel);
