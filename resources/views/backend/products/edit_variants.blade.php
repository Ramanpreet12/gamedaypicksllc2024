...
@section('subcontent')
    ...
    <div class="intro-y box mt-5">
        ...
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60 dark:border-darkmode-400">
            <h2 class="font-medium text-base mr-auto">Edit Product Variation</h2>
            <a href="{{ route('shop.edit',$product_id) }}"><button class="btn btn-primary">Back</button></a>
        </div>
        <form action="{{ url('admin/products/shop/'.$product_id.'/edit-variants/'.$variant_id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PUT')
                ...
                <div class="grid grid-cols-12 gap-4 row-gap-5 mt-5">
                            ...
                            @if (isset($get_sizes) && $get_sizes->isNotEmpty())
                                @foreach ($get_sizes as $size)
                                    <option value="{{ $size->id }}"
                                        {{ $get_product_variant->size_id == $size->id ? 'selected' : '' }}>
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        <div class="mb-2">
                            ...
                            <input id="product_price" type="text" class="form-control input w-full border flex-1"
                                placeholder="Price" name="product_price" value="{{ old('product_price' , $get_product_variant->product_price) }}">
                        </div>
                        @error('product_price')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <input id="product_quantity" type="text" class="form-control input w-full border flex-1 "
                            placeholder="Product quantity" name="product_quantity" value="{{ old('product_quantity' , $get_product_variant->product_quantity) }}">
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <select class="form-control" id="status" name="status">
                            ...
                            <option value="1" {{$get_product_variant->status == '1' ? 'selected' : ''}}>Active</option>
                            <option value="0" {{$get_product_variant->status == '0' ? 'selected' : ''}}>Inactive</option>
                        </select>
                        @error('status')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
