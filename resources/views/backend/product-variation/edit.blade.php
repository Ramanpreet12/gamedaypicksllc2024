...
@section('subcontent')
    <div class="intro-y box mt-5">
        ...
        <form action="{{ route('product-variations.update' , $product_variation->id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PUT')
                ...
                <div class="grid grid-cols-12 gap-4 row-gap-5 mt-5">
                            ...
                            @if (isset($get_products) && $get_products->isNotEmpty())
                                @foreach ($get_products as $product)
                                    <option value="{{ $product->id }}"
                                        {{ $product_variation->product_id == $product->id ? 'selected' : '' }}>
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <input id="product_size" type="text" class="form-control input w-full border flex-1"
                            placeholder="Size" name="product_size" value="{{ old('product_size' , $product_variation->product_size) }}">
                        @error('product_size')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="intro-y col-span-12 sm:col-span-6">
                        <div class="mb-2">
                            ...
                            <input id="product_price" type="text" class="form-control input w-full border flex-1"
                                placeholder="Price" name="product_price" value="{{ old('product_price' , $product_variation->product_price) }}">
                        </div>
                        @error('product_price')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <input id="product_quantity" type="text" class="form-control input w-full border flex-1 "
                            placeholder="Product quantity" name="product_quantity" value="{{ old('product_quantity' , $product_variation->product_quantity) }}">
                    ...
                    <div class="intro-y col-span-12 sm:col-span-6">
                        ...
                        <select class="form-control" id="status" name="status">
                            ...
                            <option value="1"  {{$product_variation->status ==1 ? 'selected': ''}}>Active</option>
                            <option value="0"  {{$product_variation->status ==0 ? 'selected': ''}}>Inactive</option>
                        </select>
                        @error('status')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
